...
class HierarchicalSoftmax(object):
    ...
    def __init__(self, input_, target, n_in, n_out):
        ...
        self.n_layer1_nodes = numpy.ceil(numpy.sqrt(n_out)).astype('int64')
        # and at most sqrt(n_out) nodes in the second level
        self.n_layer2_nodes = numpy.ceil(n_out/float(self.n_layer1_nodes)).astype('int64')

        # define weight matrix 'W' and bias 'b' for first layer in output tree
        self.W = theano.shared(value=numpy.zeros((n_in,  self.n_layer1_nodes), dtype=theano.config.floatX),
                               name='W', borrow=True)
        self.b = theano.shared(value=numpy.zeros((self.n_layer1_nodes,), dtype=theano.config.floatX),
                               name='b', borrow=True)

         # define weight matrix 'U' and bias 'c' for second layer in output tree
        self.U = theano.shared(value=numpy.zeros((n_in,  self.n_layer2_nodes), dtype=theano.config.floatX),
                               name='W', borrow=True)
        self.c = theano.shared(value=numpy.zeros((self.n_layer2_nodes,), dtype=theano.config.floatX),
                               name='b', borrow=True)
    ...
    def forward_prop(self, input_, target=None):
        ...
        # compute only batch_size predictions [ time complexity is O(2 x sqrt(n_out)) = O(sqrt(n_out)) ]
            ...
            level1_idx = target // self.n_layer1_nodes
            level2_idx = target % self.n_layer2_nodes
